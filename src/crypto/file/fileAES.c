...
void encryptFileInfoAES256(FileInfo* file, unsigned char *key)
{
...
void decryptFileInfoAES256(FileInfo* file, unsigned char *key)
{
    unsigned char* iv = NULL;
    memcpy(iv, file->m_fileContent, 16);
    file->m_fileSize -= 16;
    decryptTextAES256(file->m_fileContent + 16, &file->m_fileSize, key, iv);
}